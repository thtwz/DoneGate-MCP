...
def project_status(task: Task) -> TaskStatus:
    ...
    if not has_started:
        return TaskStatus.DRAFT if task.status == TaskStatus.DRAFT else TaskStatus.READY
    if task.verification_status == VerificationStatus.FAILED:
        return TaskStatus.IN_PROGRESS
    if task.verification_status != VerificationStatus.PASSED:
        return TaskStatus.IN_PROGRESS if task.status == TaskStatus.IN_PROGRESS else TaskStatus.AWAITING_VERIFICATION
...
def needs_verification(task: Task) -> bool:
    ...
    return task.needs_revalidation or task.status == TaskStatus.AWAITING_VERIFICATION


def needs_docs(task: Task) -> bool:
    task = normalize_task(task)
    return task.status == TaskStatus.VERIFIED
...
def require_transition(task: Task, target: TaskStatus) -> None:
    ...
    elif not can_transition(task.status, target):
        raise TransitionError(f"cannot move {task.task_id} from {task.status.value} to {target.value}")
